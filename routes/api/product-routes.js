    ...
    include:[
      {
        ...
        attributes: [ // include the attributes of the categories table
      ...
      },
      {
        ...
        attributes: [ // include the attributes of the tags table
          'id', // include the id of the tag
          'tag_name' // include the tag name
        ],
...
router.get('/:id', (req, res) => {
  ...
  Product.findOne({ // include: [Category, Tag] })
    where:{ // find the product with the id of the req.params.id value (the id of the product)
      id: req.params.id // find the product with the id of the req.params.id value (the id of the product)
    },
    include:[ // include the categories table (model) and include the attributes of the categories table (attributes)
      {
        model: Category, // include the categories table (model) and include the attributes of the categories table (attributes)
        attributes: [ // include the attributes of the categories table
          'id', // include the id of the category
      ...
      {
        model: Tag, // include the tags table (model) and include the attributes of the tags table (attributes)
        attributes: ['tag_name'], // include the attributes of the tags table
        through: ProductTag, // include the product_tag table (through) and include the attributes of the product_tag table (attributes)
        as: 'tags' // include the tags as the tags attribute
      }
    ]
  ...
  .then(productGetIDData => res.json(productGetIDData)) // send the data back to the client as JSON in the response body and end the response
    .catch(err => { // if there is an error
      console.warn(err, 'Status 500 code, theres an error in the JSON (product-routes GET "/:id"');
...
router.post('/', (req, res) => {
  ...
  */
  Product.create({ // create a new product in the database
    ...
    .then((product) => {
      ...
      if (req.body.tagIds.length) { // if there are product tags
        const productTagIdArr = req.body.tagIds.map((tag_id) => { // create an array of product tag ids
          return { // create a new object for each product tag id
    ...
    .then((productTagIds) => res // send a 201 status code and the product as JSON in the response body and end the response
      .status(200) // send a 200 status code and the product as JSON in the response body and end the response
      .json(productTagIds)) // end the response
    .catch((err) => { // if there is an error
      console.warn(err, 'Status 500 code, theres an error in the JSON (product-routes POST "/"');
  ...
  Product.update(
    ...
    .then((productTags) => { 
      ...
      const productTagIds = productTags
        .map(({tag_id}) => tag_id); // get list of current tag_ids
      ...
      const newProductTags = req.body.tagIds // create filtered list of new tag_ids
        ...
        .map((tag_id) => { // create a new object for each new tag_id
          return { // create a new object for each new tag_id
      ...
      // run both actions
      return Promise.all([ // run both actions in parallel
        ProductTag // run both actions and return the results
          .destroy({ // run both actions (destroy the product tags to remove)
            where: { // run both actions
              id: productTagsToRemove // run both actions
            }
          }),
        ProductTag
          .bulkCreate(newProductTags), // run both actions and return the results
      ]);
    })
    ...
    .catch((err) => {
      console.warn(err, 'Status 500 code, theres an error in the JSON (product-routes GET "/:id"'); // if there is an error
      res
        .status(400) // send a 400 status code and the error as JSON in the response body and end the response
        .json(err); // end the response
    });
...
router.delete('/:id', (req, res) => { // delete product
  ...
  Product.destroy({ // delete one product by its `id` value
    where:{ // delete one product by its `id` value
      id: req.params.id // delete one product by its `id` value
    } // delete one product by its `id` value
  })
  .then(productDeleteData => { // if there is dbCategoryData
    if (!productDeleteData) { // if there is no dbCategoryData
      res
        .status(404)
        .json({
    ...
    res
      .json(productDeleteData);
  })
  .catch(err => {
    console.warn(err, 'Status 500 code, theres an error in the JSON (product-routes DELETE "/:id"');
    res
      .status(500)
      .json(err);
  });
