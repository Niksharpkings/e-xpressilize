...
Product.init( // initialize the Product class with the sequelize constructor and the data types from the library (DataTypes) to use in our models (Model) (this is needed for the routes to work)
  {
    // define columns
    ...
    product_name: { // product_name is the name of the product (VARCHAR)
      type: DataTypes.STRING, // type is a string (STRING)
      allowNull: false // doesn't allow null values (NOT NULL)
    },
    price: { // price is the price of the product (FLOAT)
      type: DataTypes.DECIMAL(10,2), // type is a decimal (DECIMAL) with a precision of 10 and a scale of 2
      allowNull: false, // doesn't allow null values (NOT NULL)
      validate: { // validate the price field
        isNumeric: true // make sure the price is a number
      } 
    },
    stock: { // stock is the stock of the product (INTEGER)
      type: DataTypes.INTEGER, // type is an integer (INTEGER)
      allowNull: false, // doesn't allow null values (NOT NULL)
      default: 10, // default stock to 10
      validate: { // validate the stock field
        isNumeric: true // make sure the stock is a number
      }
    },
    category_id: { // category_id is the id of the category the product belongs to (INTEGER)
      type: DataTypes.INTEGER, // type is an integer (INTEGER)
      references: { // references the category table
